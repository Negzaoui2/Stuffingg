...
class RagService:
		...
		if df is not None and not df.empty:
			try:
				source_name = (
					...
					if self.settings.data_source == "dataset" and self._dataset_path
					else f"postgres:{self.settings.postgres_table}"
				)
			...
			print("[RAG] Aucune donnee chargee pour initialiser l'index")
	...
	def build_index_from_dataframe(
		...
		if self._use_semantic and self._encoder is not None:
			...
			cache_file = source_name.replace(":", "_").replace("/", "_").replace("\\", "_")
