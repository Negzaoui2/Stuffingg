...
class Settings(BaseSettings):
	...
	# Dataset / RAG
	data_source: Literal["dataset", "postgres"] = Field(
		default="dataset",
		description="Data source mode: 'dataset' (CSV) or 'postgres' (PostgreSQL).",
	)
	...
	postgres_dsn: str | None = Field(
		default=None,
		description="Full PostgreSQL DSN (e.g. postgresql+psycopg://user:pass@host:5432/db).",
	)
	postgres_table: str = Field(
		default="hr_employee_attrition",
		description="Table name to load when data_source=postgres.",
	)
	postgres_query: str | None = Field(
		default=None,
		description="Optional SQL query used instead of POSTGRES_TABLE when data_source=postgres.",
	)
